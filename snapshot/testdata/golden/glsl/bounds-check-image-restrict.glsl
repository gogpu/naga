...
vec4 test_textureLoad_1d(int coords, int level) {
    vec4 _e3 = texelFetch(_group_0_binding_0_fs, coords, level);
    return _e3;
}

vec4 test_textureLoad_2d(ivec2 coords_1, int level_1) {
    vec4 _e3 = texelFetch(_group_0_binding_1_fs, coords_1, level_1);
    return _e3;
}

vec4 test_textureLoad_2d_array_u(ivec2 coords_2, uint index, int level_2) {
    vec4 _e4 = texelFetch(_group_0_binding_2_fs, ivec3(coords_2, index), level_2);
    return _e4;
}

vec4 test_textureLoad_2d_array_s(ivec2 coords_3, int index_1, int level_3) {
    vec4 _e4 = texelFetch(_group_0_binding_2_fs, ivec3(coords_3, index_1), level_3);
    return _e4;
}

vec4 test_textureLoad_3d(ivec3 coords_4, int level_4) {
    vec4 _e3 = texelFetch(_group_0_binding_3_fs, coords_4, level_4);
    return _e3;
}

vec4 test_textureLoad_multisampled_2d(ivec2 coords_5, int _sample) {
    vec4 _e3 = texelFetch(_group_0_binding_4_fs, coords_5, _sample);
    return _e3;
}
